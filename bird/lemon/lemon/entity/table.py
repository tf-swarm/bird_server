...
from framework.interface import ILogic
...
class Table(ILogic):
    ...
    def _notify_join_table(self, uid, reconnect=False, viewer=False):
    ...
    def join_table(self, uid):
        ...
        if result != 0:
            ...
            if not self.all_user:
                self._info('join error and table empty')
    ...
    def ready(self, uid):
        ...
        result = self.on_ready(uid)
        if result == 0:
            ack = MsgPack(Message.MSG_SYS_READY | Message.ID_ACK)
            Context.GData.send_to_connect(uid, ack)

            evt = [{
                'type': Enum.table_event_ready,
                'userId': uid,
            }]
            self.notify_event(evt)
        else:
    ...
    def force_quit(self, uid):
        result = self.on_force_quit(uid)
        if result == 0:
            ack = MsgPack(Message.MSG_SYS_FORCE_QUIT | Message.ID_ACK)
            Context.GData.send_to_connect(uid, ack)

            evt = [{
                'type': Enum.table_event_force_quit,
                'userId': uid,
            }]
            self.notify_event(evt, exclude=uid)
    ...
    def viewer_join_table(self, uid, sid):
        if self.recycle_ts:
            return Enum.viewer_join_table_failed_unknown

        result = self.on_viewer_join(uid, sid)
        if result == 0:
            ack = MsgPack(Message.MSG_SYS_VIEWER_JOIN_TABLE | Message.ID_ACK)
            Context.GData.send_to_connect(uid, ack)

            self._notify_join_table(uid, viewer=True)
        return result

    def viewer_leave_table(self, uid):
        result = self.on_viewer_leave(uid)
        if result == 0:
            ack = MsgPack(Message.MSG_SYS_VIEWER_LEAVE_TABLE | Message.ID_ACK)
            Context.GData.send_to_connect(uid, ack)
            evt = [{
                'type': Enum.table_event_viewer_leave_table,
                'userId': uid,
            }]
            self.notify_event(evt, exclude=uid)
        return result

    def broadcast(self, uid, msg):
        result = self.on_broadcast(uid, msg)
        if result == 0:
            ack = MsgPack(Message.MSG_SYS_BROADCAST | Message.ID_ACK)
            Context.GData.send_to_connect(uid, ack)

        self.table_broadcast(msg)
        return result

    def flush(self, uid, msg):
        result = self.on_flush(uid, msg)
        if result == 0:
    ...
    def trustee(self, uid, cancel):
        if cancel:
            result = self.on_cancel_trustee(uid)
        else:
            result = self.on_trustee(uid)
    ...
    def __reconnect(self, uid):
        self._notify_join_table(uid, reconnect=True)
        self.on_reconnect(uid)
        return 0

    def reconnect(self, uid):
        state = self.get_user_state(uid)
        self._info('reconnect with user state', state)
        result = Enum.reconnect_failed_state
        if state == Enum.user_state_offline or state == Enum.user_state_getout:
            result = self.__reconnect(uid)
        return result
    ...
    def set_recycle_flag(self):
        ...
        Context.Log.info('set_recycle_flag2')
        registry = Context.get_module(self.gid, 'registry')
        registry.remove_table(self.tid)
