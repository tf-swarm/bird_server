    ...
    def on_task_list(self, uid, gid, mi):
        ...
        if task_list:
            ...
            if total_degree < conf['total_degree']:
                bird_config = Context.Configure.get_game_item_json(gid, 'bird.config')
                bird_config = Context.copy_json_obj(bird_config)
                while total_degree < conf['total_degree']:  # 普通鸟填充, 直到达到最大活跃值
                    task_type = 1
                    task = {'id': len(task_list), 'type': task_type}
                    index = random.randrange(0, len(bird_config['common']))
                    bird_type = bird_config['common'][index]
                    del bird_config['common'][index]
                    task['total'] = random.randint(*task_map[task_type]['range'])
                    task['desc'] = task_map[task_type]['desc']
                    task['bird_type'] = bird_type
        ...
        for task in task_list:
            if task['type'] == 1:
                count = Context.Daily.get_daily_data(uid, gid, 'bird.' + str(task['bird_type']))
        ...
        for _state, reward in zip(_states, conf['reward']):
            ...
            _reward = props.BirdProps.convert_reward(reward['reward'])
    ...
    def on_consume_task(self, uid, gid, mi):
        ...
        for i, reward in enumerate(conf['reward']):
            if i == _id:
                ...
                reward = props.BirdProps.issue_rewards(uid, gid, reward['reward'], 'task.reward.%d' % _id, True)
                _reward = props.BirdProps.convert_reward(reward)
