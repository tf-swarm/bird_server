...
class CDKeyTasklet(server.BasicHttpTasklet):
    def onMessage(self, *args, **kwargs):
        from cdkey.route import HttpCdKey
        return HttpCdKey.onMessage(self.request)


class YYBTasklet(server.BasicHttpTasklet):
    def onMessage(self, *args, **kwargs):
        from yyb.route import HttpYYB
        return HttpYYB.onMessage(self.request)
