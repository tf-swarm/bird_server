...
class ConfigHelper(object):
    ...
    def add_game_config(cls, gameId, attr, value, force=True):
        if force is True or not cls.get_config_db(gameId, attr, None):
            cls.__add_config_item('configitem', 'game:%d:%s' % (gameId, attr), value)

    @classmethod
    def get_config_db(cls, gameId, attr, default=None):
        val = Context.Configure.get_game_item_json(gameId, attr)
        if val is None:
            return default
        else:
            return val
