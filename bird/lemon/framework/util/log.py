...
logging.addLevelName(logging.NOTSET, ' | N | ' + _NOTSET)   # 定制该等级日志对应日志名
...
class LogFormatter(logging.Formatter):       # 设置日志时间输出格式，精确到毫秒
...
class Logger(object):               # 日志打印类,封装python logging
    DEBUG = logging.DEBUG           # 日志级别
    INFO = logging.INFO
    WARN = logging.WARN
    ERROR = logging.ERROR
    BI = logging.FATAL

    _show_task_id = True            # log 线程id 开关
    _show_prefix = False            # log 相关前缀 开关
    _show_network = True            # log 网络 开关
    _show_redis = True              # log redis 开关
    _open_std_log = False           # log 控制台输出 开关
    _open_log = False               # log 文件输出 开关
    _open_bi_log = False            # log 系统级日志 开关
    ...
    def testlog(cls, *args):
        ...
        fstr += "\n"
        file = os.path.dirname(Global.log_path()) + "/test.log"
