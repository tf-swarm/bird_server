...
class Service(object):
    def __init__(self, innerProtocol=None, outerProtocol=None, httpFactory=None, wsFactory=None, httpsFactory=None):
        self.__heart_beat = 0
    ...
    def start(self):
        ...
        Context.Log.info('InitParams httpsFactory  =', self.httpsFactory)
    ...
    def start_up(self):
        ...
        elif self.httpsFactory:
            self.listen_https(self.listen_port)

        if self.outerProtocol:
            self.listen_tcp(self.listen_port + 1)
        elif self.wsFactory:
            self.listen_websocket(self.listen_port + 1)
    ...
    def listen_http(self, port):
        ...
        if Context.Global.run_mode() == 1:
            reactor.listenTCP(port, self.httpFactory(), interface='127.0.0.1')
        else:
            reactor.listenTCP(port, self.httpFactory())

    def listen_https(self, port):
        type_str = FlagType.trans_server_type(Context.GData.server_type)
        Context.Log.info(type_str, 'listen on port', port, 'with', self.httpsFactory)

        sslContext = ssl.DefaultOpenSSLContextFactory(
            '/home/yyb-file/private.key',        # 私钥
            '/home/yyb-file/yyb.crt'             # 证书
        )
        if Context.Global.run_mode() == 1:
            reactor.listenSSL(port, self.httpsFactory(), sslContext, interface='127.0.0.1')
        else:
            ret = reactor.listenSSL(port, self.httpsFactory(), sslContext)
            Context.Log.debug('listenSSL ret', ret)


    def listen_websocket(self, port):
        type_str = FlagType.trans_server_type(Context.GData.server_type)
        Context.Log.info(type_str, 'listen on port', port, 'with', self.wsFactory)
        reactor.listenTCP(port, self.wsFactory())
