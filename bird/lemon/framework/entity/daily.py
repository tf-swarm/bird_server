...
class Daily(ICallable, IContext):
    ...
    def incr_daily_data_default(self, uid, gid, field, delta=1, default = 0):
        key = 'daily:%d:%d' % (gid, uid)
        tomorrow_ts = Time.tomorrow_start_ts()
        isNew = self.ctx.RedisCluster.hash_setnx(uid, key, 'expire_ts', tomorrow_ts)
        if isNew:
            self.ctx.RedisCluster.hash_incrby(uid, key, field, delta + default)
            self.ctx.RedisCluster.expire_at(uid, key, tomorrow_ts)
            return delta + default
        else:
            return self.ctx.RedisCluster.hash_incrby(uid, key, field, delta)

    def get_daily_data_default(self, uid, gid, field, default=0):
        key = 'daily:%d:%d' % (gid, uid)
        tomorrow_ts = Time.tomorrow_start_ts()
        isNew = self.ctx.RedisCluster.hash_setnx(uid, key, 'expire_ts', tomorrow_ts)
        if isNew:
            self.ctx.RedisCluster.hash_set(uid, key, field, default)
            self.ctx.RedisCluster.expire_at(uid, key, tomorrow_ts)
            return default
        else:
            is_item_New = self.ctx.RedisCluster.hash_setnx(uid, key, field, default)
            if is_item_New:
                return default
            else:
                return self.ctx.RedisCluster.hash_get(uid, key, field)
