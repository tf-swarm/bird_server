...
class GData(IContext, ICallable):
    def __init__(self):
        ...
        self.login_activity_user = set([])   # 当前小时在线过的uid（只要在线过就算）
        self.online_user = {}
        self.online_table = {}
    ...
    def judge_user_connect(self, uid):
    ...
    def __send_to_server(self, uid, msg, dst, sid, cmd, gid, pipe = None):
        if sid is None:
        ...
        mo = MsgLine(msg, gid, target=uid, pipe = pipe)
        return conn.sendMsg(cmd, mo.pack())

    def send_to_connect(self, uid, msg, sid=None, cmd=None, gid=None, pipe = None):
        return self.__send_to_server(uid, msg, FlagType.flag_type_connect, sid, cmd, gid, pipe= pipe)
    ...
    def send_to_proxy(self, uid, msg, sid=None, cmd=None, gid=None):
        return self.__send_to_server(uid, msg, FlagType.flag_type_proxy, sid, cmd, gid)
    ...
    def get_new_user_id(self):
        self.ctx.RedisMix.hash_setnx('global.info.hash', 'max.user.id', 1000000)
        max_user_id = self.ctx.RedisMix.hash_incrby('global.info.hash', 'max.user.id', 1)
        self.ctx.Log.report('max.user.id:', max_user_id)
        return max_user_id

    def get_new_led_id(self):
        self.ctx.RedisMix.hash_setnx('global.info.hash', 'max.led.id', 100)
        max_led_id = self.ctx.RedisMix.hash_incrby('global.info.hash', 'max.led.id', 1)
        self.ctx.Log.report('max.led.id:', max_led_id)
        return max_led_id

    def get_new_village_id(self):
        self.ctx.RedisMix.hash_setnx('global.info.hash', 'max.village.id', 10000)
        max_village_id = self.ctx.RedisMix.hash_incrby('global.info.hash', 'max.village.id', 1)
        self.ctx.Log.report('max.user.id:', max_village_id)
        return max_village_id

    def user_is_online(self, uid):
        if uid in self.map_client_connect:
            return True
        return False
