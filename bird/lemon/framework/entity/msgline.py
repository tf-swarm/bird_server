...
class MsgLine(object):
    ...
    def __init__(self, raw, gid=None, room=None, target=None, pipe = None):
        self.target = target
        self.gid = gid
        self.room = room
        self.raw = raw
    ...
    @classmethod
    def unpack(cls, line):
        _gid, _room, _pipe, _target, _msg = line.split(cls.FIELD_SEPARATOR, 5)
        if _gid:
            _gid = int(_gid)
        if _room:
            _room = int(_room)
        ...
        return MsgLine(_msg, _gid, _room, target, _pipe)
