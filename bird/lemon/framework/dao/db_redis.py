...
class DbRedis(object):
    ...
    def connect(self, *args):
        ...
        if self.__redis_key__ in self.__CONNECT_REDIS__:
            ...
            # pool = redis.ConnectionPool(host=host, port=port, dbid=db)
            # self.__redisPool__ = tasklet.wait_for_deferred(pool)
    ...
    def set_range(self, key):
        return self.execute('SUNION', key)
    ...
    def hget_keys(self, key):# 这里还是得用1000，越大会卡
        a, b = self.execute('scan', 0, 'match', key, 'count', 1000)
        while a != '0':
            a, b1 = self.execute('scan', a, 'match', key, 'count', 1000)
            b.extend(b1)
        return b