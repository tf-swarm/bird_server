...
class SdkSession(BasicSession):
    ...
    def __cache_session(self):
        ...
        Context.RedisCache.setex('session:%s' % self.uid, data, int(self.sessionTimeout))
...
class SdkHttpFactory(ServerHttpFactory):
    ...
    def getSession(self, uid):
        if uid not in self.sessions:
            from framework.context import Context
            data = Context.RedisCache.get('session:%s' % uid)
            if data:
...
class CDKeyHttpFactory(ServerHttpFactory):
    pass


class YYBHttpsFactory(ServerHttpFactory):
    pass
