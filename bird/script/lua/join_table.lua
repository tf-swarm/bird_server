...
local function join_table(uid, gid, rid, tid, total, mode)
    local key = 'relax_table:' .. gid .. ':' .. tid
    local before_cnt = get_player_count(key, total)
    local seatid = sit_down(uid, key, total)

    local before_key = 'relax_quick:' .. gid .. ':0'
    if before_cnt ~= 0 then
        if mode ~= nil then
            before_key = 'relax_quick:' .. gid .. ':' .. rid .. ':' .. mode
        else
            before_key = 'relax_quick:' .. gid .. ':' .. rid
        end
    end

    local after_cnt = get_player_count(key, total)
    local after_key = 'relax_quick:' .. gid .. ':0'
    if after_cnt ~= 0 then
        if mode ~= nil then
            after_key = 'relax_quick:' .. gid .. ':' .. rid .. ':' .. mode
        else
            after_key = 'relax_quick:' .. gid .. ':' .. rid
        end
    end

    if before_cnt ~= after_cnt then
        redis.call('SREM', before_key, '0', tid)
        redis.call('SADD', after_key, tid)
    end
