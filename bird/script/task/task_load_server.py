...
from framework.helper import Util
...
def action_load_server(params):
    ...
    for process in server['process']:
        ...
        if process['type'] == 'sdk':
            ...
            http_port += 2
        elif process['type'] == 'cdkey':
            process['port'] = http_port
            http_port += 2
        elif process['type'] == 'yyb':
            process['port'] = http_port
            http_port += 2
        else:
            process['port'] = tcp_port
            tcp_port += 2
        if is_local_host(server, process):
            process['local'] = 1
...
def is_local_host(server, process):
    host = server['servers'][process['server']]
    return Util.is_local_ip(host['intranet']) or Util.is_local_ip(host['internet'])
...
def __build_iptable(processs):
    iptables = []
    for p in processs:
        if p['type'] == 'connect':
            s = 'sudo iptables -A INPUT -p tcp -m state --state NEW -m tcp --dport %s -j ACCEPT' % (p['port'] + 1)
            iptables.append(s)
    return iptables
...
def game_load_server(params):
    ...
    # params['iptable'] = __build_iptable(params['server']['process'])
...
def cdkey_load_server(params):
    action_load_server(params)
    __build_server_info(params)

shell_load_server = sdk_load_server

yyb_load_server = sdk_load_server
